...
export class WorkItemTypeView extends BaseFluxComponent<IWorkItemTypeViewProps, IWorkItemTypeViewState> {
    ...
    public render(): JSX.Element {
        return (
            <Hub className="rule-group-list-hub" hubViewState={this._hubViewState} hideFullScreenToggle={true} commands={this._getHubCommands()}>
    ...
    private _renderChecklistView(): JSX.Element {
        ...
        return (
            <div className="checklist-view">
                ...
                {this._renderError()}
                <div className="checklist-items-container">{this._renderChecklistItems()}</div>
                <ChecklistItemEditor inputPlaceholder="Add new item" disabled={disabled} onSubmit={this._addChecklistItem} />
            </div>
        );
    }
        ...
        if (this.state.editItem) {
            return (
                <Modal isOpen={true} onDismiss={this._cancelItemEdit} isBlocking={false} containerClassName="edit-checklist-item-overlay">
                    <ChecklistItemEditor checklistItem={this.state.editItem} onSubmit={this._updateChecklistItem} onCancel={this._cancelItemEdit} autoFocus={true} />
                </Modal>
            );
        }
        return null;
    ...
    private _renderChecklistItems(): JSX.Element {
        ...
        if (checklist.checklistItems != null && checklist.checklistItems.length > 0) {
            ...
            return <SortableList items={items} axis="y" lockAxis="y" onSortEnd={this._reorderChecklistItem} useDragHandle={true} />;
        }
        return null;
    }
